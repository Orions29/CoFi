...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add_cafe_attempt') {
    ...
    try {
        ...
        $sqlCafe = "INSERT INTO cafes (cafe_name, cafe_email, latitude, longitude, cafe_description, price_min, price_max, price_avg) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)";

        $stmtCafe = $sqlConn->prepare($sqlCafe);

        // NOTE - Tipe data: s=string, d=double/decimal
        // NOTE - Urutan: Name(s), Email(s), Lat(d), Long(d), Desc(s), Min(d), Max(d), Avg(d)
        $stmtCafe->bind_param(
            "ssddsddd",
            $_POST['namaCafeAdd'],
            $_POST['cafeEmailAdd'],
        ...
        $stmtCafe->execute();
        $newCafeId = $sqlConn->insert_id; // AMBIL ID CAFE BARU
